...
- **時刻查詢**：選擇起訖站與日期，查詢當日 OD 直達班次，顯示車種、發車／到達時間與行駛時間，可一鍵交換起訖站；班次依發車時間排序，並可用時間範圍或時段快捷（清晨／早上／下午／晚上）即時篩選。
...
├── components/   # BottomNav 底部導航、StationInput 車站選擇、TimeInput 時間輸入
